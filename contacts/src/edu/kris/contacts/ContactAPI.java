...
public class ContactAPI {
	...
	private static ContactAPI api;
 	...
 	private boolean insertContactData(ArrayList <RawContact> people, MainActivity.ContactTask task) {
		...
		long now = System.currentTimeMillis();
 		int peopleSize = people.size();
 		Log.d(TAG, "Will insert data for " + peopleSize + " contacts.");
 		
 		// Bulk insert the data  
 		for(int i = 0; i < peopleSize; i++) {
 			contact = people.get(i);
 			rawContactID = contact.getId();
 			if(task != null) { task.updateProgress((i*100) / peopleSize); }
 			...
 			for(int j = 0; j < contact.getEmail().size(); j++){
 				if(contact.getEmail().get(j).getType() > -1){
 	 	 			...
 	 	 			content.put(ContactsContract.Data.RAW_CONTACT_ID, rawContactID);
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " email added.");
 			...
 			for(int j = 0; j < contact.getEvents().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Event added.");
 			...
 			for(int j = 0; j < contact.getImAddresses().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Im added.");
			...
			if(contact.getNickName() != null && contact.getNickName().getType() > -1){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Nickname added.");
 			...
 			for(int j = 0; j < contact.getNotes().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Note added.");
			...
			if(contact.getOrganization() != null && contact.getOrganization().getType() > -1){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Organization added.");
 			...
 			for(int j = 0; j < contact.getPhone().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Phone added.");
			...
			if(contact.getRelation() != null && contact.getRelation().getType() > -1){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " Relation added.");
 			...
 			for(int j = 0; j < contact.getSips().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " SipAddress added.");
			...
			if(contact.getStructuredName() != null){
 	 			...
 	 			if(contact.getStructuredName().getGivenName() != null){
 	 				content.put(ContactsContract.CommonDataKinds.StructuredName.DISPLAY_NAME, 	
 	 						contact.getStructuredName().getGivenName() + " " + contact.getStructuredName().getFamilyName());	
 	 			}
 	 			if(contact.getStructuredName().getGivenName() != null){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " StructuredName added | " + (content != null ? content.toString() : "null"));
 			...
 			for(int j = 0; j < contact.getAddresses().size(); j++){
 	 		...
 	 		Log.v(TAG, rawContactID +  ") " + values.size()  + " StructuredPostal added.");
 			...
 			for(int j = 0; j < contact.getWebsites().size(); j++){
 		 	...
 		 	Log.v(TAG, rawContactID +  ") " + values.size()  + " Website added.");
 		...
 		for(int i=0; i < contentArray.length; i++){
 	 		Log.v(TAG, i +  ") will insert: " + contentArray[i].toString());
 		}
 		success = cr.bulkInsert( ContactsContract.Data.CONTENT_URI, contentArray) > 0;
 		Log.i(TAG, "INSERT PEOPLE DATA " + (System.currentTimeMillis() - now) + " milliseconds.");
 	...
 	private int insertEmptyRawContacts(ArrayList <RawContact> people) {
 		
 		int success = 0;
 		String displayName = "";
 		int numberOfContacts = people.size();
 		Account csvAccount =  getDefaultAccount();

 		// Insert the raw contacts
 		if(numberOfContacts > 0){
 	 		
 			ContentValues[] values = new ContentValues[numberOfContacts];
 	 		for(int i = 0; i < numberOfContacts; i++) {
 	 			// Get a proper display name
 	 			displayName = people.get(i).getDisplayName();
 	 			if(displayName == null || displayName.equals("")){
 	 				displayName = people.get(i).getStructuredName().getGivenName() 
 	 						+ " " + people.get(i).getStructuredName().getFamilyName();
 	 			}
 	 			...
 	 			content.put(ContactsContract.Data.DISPLAY_NAME, displayName);
 	 			values[i] = content;  
 	 		}
